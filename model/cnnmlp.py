...
for train_index, test_index in kf.split(statics, groups):
    ...
    # Train SVM
    mlp_clf = MLPClassifier(hidden_layer_sizes=(64, 16), max_iter=3000)
    mlp_clf.fit(combined_features_train, groups[train_index].numpy())
    ...
    preds_mlp = mlp_clf.predict(combined_features_test)
    accuracy_mlp = accuracy_score(groups[test_index].numpy(), preds_mlp)
    # 计算混淆矩阵
    confusion_mat = confusion_matrix(groups[test_index].numpy(), preds_mlp)
