...
for train_index, test_index in kf.split(statics, groups):
    ...
    linear_reg.fit(combined_features_train, groups[train_index].numpy())
    ...
    # 使用线性回归模型进行预测
    preds_linear_reg = linear_reg.predict(combined_features_test)
    
    # 将回归输出映射到二进制类别（0和1）
    #preds_linear_reg = np.round(preds_linear_reg)
    threshold = 0.5  # 设置阈值
    preds_linear_reg = (preds_linear_reg > threshold).astype(int)
    
    accuracy_linear_reg = accuracy_score(groups[test_index].numpy(), preds_linear_reg)
     # 计算混淆矩阵
    confusion_mat = confusion_matrix(groups[test_index].numpy(), preds_linear_reg)
